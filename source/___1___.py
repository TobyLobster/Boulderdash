...
config.set_inline_comment_column(88)
...
substitute_labels = {
    ...
    (0x2944, 0x299e): {
        "sound0_active_flag": "data_set_ptr_low",
    },
    (0x2c80, 0x2cec): {
        "data_set_ptr_low": "sound_active_flag_table",
    },
...
label(0x006f, "bonus_life_available_flag")
...
for i in map_cell_types:
    constant(i, map_cell_types[i])
...
def map_comments(addr):
    for i in range(16):
        byte(addr+i)
        comment(addr+i, map_cell_types[i], inline=True)
def cave_comments(addr):
    for i in range(20):
        byte(addr+i)
...
for i in range(16):
    comment(0x1f80+i, f"cell type ${(i+16*0):0>2X} = {map_cell_types[i]}", inline=True)
    if i == 8:
        comment(0x1f90+i, f"cell type ${(i+16*1):0>2X} = map_active_exit", inline=True)
    elif i == 3:
        comment(0x1f90+i, f"cell type ${(i+16*1):0>2X} = map_large_explosion_state1", inline=True)
    else:
        comment(0x1f90+i, f"cell type ${(i+16*1):0>2X} = {map_cell_types[i]} | map_anim_state1", inline=True)
    if i == 3:
        comment(0x1fa0+i, f"cell type ${(i+16*2):0>2X} = map_large_explosion_state2", inline=True)
    else:
        comment(0x1fa0+i, f"cell type ${(i+16*2):0>2X} = {map_cell_types[i]} | map_anim_state2", inline=True)
    if i == 3:
        comment(0x1fb0+i, f"cell type ${(i+16*3):0>2X} = map_large_explosion_state3", inline=True)
    else:
        comment(0x1fb0+i, f"cell type ${(i+16*3):0>2X} = {map_cell_types[i]} | map_anim_state3", inline=True)
    if i == 6:
        comment(0x1fc0+i, f"cell type ${(i+16*4):0>2X} = map_start_large_explosion", inline=True)
    else:
        comment(0x1fc0+i, f"cell type ${(i+16*4):0>2X} = {map_cell_types[i]} | map_anim_state4", inline=True)
    comment(0x1fd0+i, f"cell type ${(i+16*5):0>2X} = {map_cell_types[i]} | map_anim_state5", inline=True)
    comment(0x1fe0+i, f"cell type ${(i+16*6):0>2X} = {map_cell_types[i]} | map_anim_state6", inline=True)
    comment(0x1ff0+i, f"cell type ${(i+16*7):0>2X} = {map_cell_types[i]} | map_anim_state7", inline=True)
message(0x1f80, 0x2000)
label(0x1f87, "fungus_animated_sprite0")
label(0x1f8f, "rockford_sprite")
#label(0x1f87+16, "fungus_animated_sprite1")
#label(0x1f87+16*2, "fungus_animated_sprite2")
#label(0x1f87+16*3, "fungus_animated_sprite3")
label(0x1fc7, "fungus_animated_sprites4")
#label(0x1fc7+16, "fungus_animated_sprites5")
#label(0x1fc7+16*2, "fungus_animated_sprites6")
#label(0x1fc7+16*3, "fungus_animated_sprites7")
for i in range(0x1f80, 0x2000, 16):
    blank(i)
stars(0x1f80, "Table to convert a cell type 0-$7f into a sprite number.\n\nNot all possible cell types are used (see the top of file for the valid cell types).\nBy changing the entries of this table on the fly, this table allows the sprite to animate without the underlying cell type needing to change.")
...
expr(0x211a, make_or(make_or("map_unprocessed", "map_anim_state0"), "map_firefly"))
expr(0x211b, make_or(make_or("map_unprocessed", "map_anim_state0"), "map_butterfly"))
expr(0x211c, make_or(make_or("map_unprocessed", "map_anim_state1"), "map_firefly"))
expr(0x211d, make_or(make_or("map_unprocessed", "map_anim_state1"), "map_butterfly"))
expr(0x211e, make_or(make_or("map_unprocessed", "map_anim_state2"), "map_firefly"))
expr(0x211f, make_or(make_or("map_unprocessed", "map_anim_state2"), "map_butterfly"))
blank(0x2120)
#byte(0x2120, 16, 1)
map_comments(0x2120)
...
map_comments(0x2140)
expr(0x2150, "map_diamond")
expr(0x2151, make_or("map_anim_state4", "map_diamond"))
expr(0x2152, "map_firefly")
expr(0x2153, make_or("map_anim_state1", "map_firefly"))
expr(0x2154, make_or("map_anim_state2", "map_firefly"))
expr(0x2155, make_or("map_anim_state3", "map_firefly"))
expr(0x2156, "map_active_exit")
expr(0x2157, make_or("map_anim_state1", "map_magic_wall"))
expr(0x2158, "map_butterfly")
expr(0x2159, make_or("map_anim_state1", "map_butterfly"))
expr(0x215a, make_or("map_anim_state2", "map_butterfly"))
expr(0x215b, make_or("map_anim_state3", "map_butterfly"))
expr(0x215c, make_or("map_anim_state2", "map_rockford"))
expr(0x215d, make_or("map_anim_state1", "map_rockford"))
expr(0x215e, "map_firefly_in_earth_box")
blank(0x2150)
byte(0x2150, 6, 1)
byte(0x2156, 9, 1)
comment(0x2150, "these are the cell types (indices into the table 'cell_type_to_sprite') that update every tick due to animation")
label(0x2150, "cell_types_that_always_animate")
#expr(0x2150, "map_diamond")
...
stars(0x21e0, True)
label(0x21e0, "explosion_replacements")
for i in range(16):
    byte(0x21e0+i)
expr(0x21e0, make_or("map_rockford","map_unprocessed"))
expr(0x21e1, make_or("map_rockford","map_unprocessed"))
expr(0x21e2, make_or("map_diamond","map_unprocessed"))
expr(0x21e3, "map_space")
blank(0x21f0)
label(0x21f0, "collision_for_cell_type")
comment(0x21f0, "Given a cell type, get the type of collision:\n$ff means rockford can move onto the cell freely (e.g. space, earth),\n$0 means no movement possible (e.g. wall), and\n$1 means move with a push (e.g rock)")
map_comments(0x21f0)
...
label(0x2204, "check_for_rock_direction_offsets")
comment(0x2204, """Given a direction (0-3), return an offset from the current position ($41) in the map to check is clear when moving a rock (or zero if direction is not possible):
   00 01 02
3f 40 41 42 43
   80 81 82
      c1""")
label(0x2208, "map_offset_for_direction")
byte(0x2208, 4)
unused(0x220c)
blank(0x220c)
byte(0x220c, 16)
...
stars(0x2256)
comment(0x2256, "Clears the entire map to initial_cell_fill_value.\nClears the visible grid to $ff\namount_to_increment_ptr_minus_one is always zero in this routine, making some of the instructions unused.")
...
comment(0x2264, "variable is always zero in practice (see calling function)", inline=True)
unused(0x2268)
...
entry(0x22a5)
comment(0x22ad, "cell is in the range $90-$9f (corresponding to $10 to $1f with the top bit set), so we look up the replacement in a table. This is used to replace the final step of an explosion, either with rockford during the introduction (offset $01), or a space for the outro (death) explosion (offset $03)", indent=1)
...
comment(0x2355, "Y=9 corresponds to the titanium wall sprite used while revealing the grid", indent=1)
comment(0x2357, "this next instruction is either:\n    'ldy cell_type_to_sprite' which in this context is equivalent to a no-op, which is used during preprocessing OR \n    'lda cell_type_to_sprite,y'\nto convert the cell into a sprite (used during actual gameplay).\nSelf-modifying code above sets which version is to be used.", indent=1)
...
stars(0x2400, """This is the map processing that happens every tick during gameplay.
The map is scanned to handle any changes required.

The offsets within the map are stored in the Y register, with the current entry having offset $41:
...
expr(0x2401, "update_rock_or_diamond_that_can_fall - branch_instruction - 2")
ab(0x2402, True)
...
comment(0x2439, "if we have no handler for this cell type then branch (destination was set depending on where we entered this routine)", indent=1)
...
stars(0x249a, "This is the update when we find a diamond or rock in the map during gameplay")
...
comment(0x24c0, "cell right is empty, now check below right cell", indent=1)
comment(0x24c6, "take the rock or diamond, and set bit 6 to indicate it has been moved this scan (so it won't be moved again). Then store it in the below left or below right cell", indent=1)
label(0x24c6, "rock_or_diamond_can_fall_left_or_right")
comment(0x24cc, "below left or right is set to $80, still a space, but marked as unprocessed", indent=1)
comment(0x24c9, "Store in either cell_below_left or cell_below_right depending on Y=$80 or $82, since $fff6 = cell_below_left - $80", indent=1)
label(0x24d0, "set_to_unprocessed_space")
ab(0x24d2, True)
label(0x24d4, "cell_below_is_a_space")
comment(0x24d4, "take the rock or diamond, and set bit 6 to indicate it has been moved this scan (so it won't be moved again). Then store it in the cell below.", indent=1)
ab(0x24d9, True)
...
expr(0x2615, "map_rockford")
label(0x2618, "play_movement_sound_and_update_current_position_address")
label(0x2626, "direction_key_pressed")
label(0x262b, "get_direction_index_loop")
label(0x2636, "skip_storing_rockford_cell_type")
comment(0x263b, "read cell contents from the given neighbouring cell variable y", indent=1)
comment(0x2643, "branch if movement is not possible", indent=1)
comment(0x2648, "branch if movement is freely possible", indent=1)
comment(0x264a, "trying to move into something difficult to move (e.g. a rock)", indent=1)
comment(0x2655, "don't try pushing a rock that's just fallen this tick (bit 6 set at $24c7)", indent=1)
label(0x2667, "check_for_return_pressed")
comment(0x266d, "return and direction is pressed. clear the appropriate cell", indent=1)
label(0x2674, "check_if_value_is_empty")
label(0x2678, "store_rockford_cell_value_without_return_pressed")
ab(0x2687, True)
stars(0x2689)
...
blank(0x2904)
comment(0x2904, "STEP 1: The initial fill. Find the cell type to fill the map with initially, and clear the map to that value", indent=1)
blank(0x2910)
comment(0x2910, "STEP 2: The 'basics'. This is a 2-bit-per-cell array that's copied into the map. One value skips the cell so the initial fill value remains, the other three are specified in the high nybbles in the cave colour arrays. Here we copy them into cell_above_left/cell_above/cell_above_right", indent=1)
...
comment(0x2927, "STEP 2a: Find the right data set to use for the cave and difficulty level", indent=1)
comment(0x2927, "look up which data set is needed from the cave number", indent=1)
comment(0x292c, "branch if no data set is present", indent=1)
...
comment(0x2942, "remember pointer to data set", indent=1)
label(0x2942, "got_data_set_X_address")
comment(0x2946, "set offset in Y = 4*(difficulty level-1)", indent=1)
comment(0x294d, "STEP 2b: Now we have found the right data set, decode the four byte data set entry", indent=1)
...
comment(0x2966, "STEP 2c: now we have the correct data, actually fill with the basics", indent=1)
blank(0x2969)
comment(0x2969, "STEP 3: The 'patches'. Individual cells can be changed using patch data.", indent=1)
comment(0x2969, "First reset ptr to the start of the data_set_X", indent=1)
comment(0x2971, "recall Y and read the next two bytes as an address. Can be zero. For patches.", indent=1)
comment(0x297b, "branch if no patch data is present", indent=1)
blank(0x2982)
comment(0x2982, "STEP 4: The 'strips'", indent=1)
...
blank(0x29ac)
comment(0x29ac, "STEP 5: Set palette", indent=1)
label(0x29ac, "set_palette")
label(0x29b0, "set_palette_loop")
expr(0x29bb, "total_caves")
stars(0x29c3)
unused_entry(0x29c3)
nonentry(0x29d2)
...
comment(0x2a84, "a bonus life only becomes possible after the score *doesn't* have a zero or five in the hundreds column", indent=1)
...
unused(0x2bc0)
comment(0x2bca, """store earth ('*' in the diagram below) in the following locations around the current position, and clear the others, and add the firefly ('+') or butterfly at the current location:
00* 01* 02* 03*
40* 41+ 42  43*
80* 81  82  83*
C0* C1* C2* C3*""")
...
stars(0x2d00, "Each byte holds a repeat count in the top nybble, and the value to store in the lower nybble (with special values $Xa meaning skip to the start of the next row X times.)")
...
comment(0x2d20, "a value of 10 means move to the next row (repeatedly)", indent=1)
comment(0x2d24, "if it's this cave's skip value, then don't write to the map", indent=1)
comment(0x2d28, "write patch byte to map", indent=1)
...
comment(0x2e03, "a bonus life only becomes possible after the score *doesn't* have a zero or five in the hundreds column", indent=1)
decimal(0x2e0f)
...
blank(0x2ebd)
label(0x2ebd, "play_screen_dissolve_to_solid")
...
stars(0x31c0, "Entry point")
entry(0x31c0, "entry_point")
comment(0x31c0, "copy 256 bytes which is the credits text into a different location. Since both source and destination are within the bounds of this file, there is no reason why this couldn't just be loaded in the correct location to start with.", indent=1)
label(0x31c2, "copy_credits_loop")
label(0x31cb, "main_menu_loop")
expr(0x31cc, make_hi("regular_status_bar"))
comment(0x31d3, "show credits", indent=1)
...
cave_comments(0x4c04)
#ten_by_two(0x4c04)
label(0x4c18, "end_y")
cave_comments(0x4c18)
#ten_by_two(0x4c18)
label(0x4c2c, "end_x")
cave_comments(0x4c2c)
#ten_by_two(0x4c2c)
label(0x4c40, "cave_play_order")
for i in range(20):
    decimal(0x4c40 + i)
    decimal(0x4c54 + i)
cave_comments(0x4c40)
#ten_by_two(0x4c40)
label(0x4c54, "fungus_growth_intervals_for_cave")
cave_comments(0x4c54)
